...
class TradingSimulator:
    ...
    def simulateNewStrategy(self, strategyName, stockName,
        ...
        if ai:
            ...
            tradingStrategy = getattr(globals(), strategy)()
        ...
        if(saveStrategy):
            fileName = "".join(["models/", strategy, "_", stock, "_", startingDate, "_", splitingDate])
            if ai:
...
class PerformanceEstimator:
    ...
    def computeAnnualizedReturn(self):
        cumulativeReturn = self.data['returns'].cumsum()[-1]
    ...
    def computeProfitability(self):
        ...
        if bad != 0:
            loss /= bad
        self.averageProfitLossRatio = profit / loss if loss != 0 else float('Inf')
        return self.profitability, self.averageProfitLossRatio
    ...
    def getComputedPerformance(self):
        ...
        data = {
            'Metric': [
                "PnL", "Annualized Return", "Annualized Volatility",
            ...
            'Value': [
                self.PnL, self.annualizedReturn / 100, self.annualizedVolatily / 100,
